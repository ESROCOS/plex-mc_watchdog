...
INTERFACEVIEW=mc_watchdog_iv.aadl
grep "version => \"2" mc_watchdog_iv.aadl >/dev/null && {
    echo -e "${INFO} Converting interface view from V2 to V1.3"
    TASTE --load-interface-view mc_watchdog_iv.aadl --export-interface-view-to-1_3 __iv_1_3.aadl
...
cd "$SKELS" && rm -f dummy_producer.zip && zip dummy_producer dummy_producer/* && cd $OLDPWD
...
cd "$CWD" && assert-builder-ocarina.py \
	...
	-o "$OUTPUTDIR" \
	--subCPP watchdog:watchdog.zip \
	--subCPP dummy_producer:"$SKELS"/dummy_producer.zip \
